...
module.exports = class Server {
  constructor(gameClass) {
    ...
    app.use(session({resave: false, saveUninitialized: false, secret: "some secret"}));
    ...
    app.post('/users', (req, res) => {
      ...
      bcrypt.hash(password, 8, (err, hash) => {
        ...
        this.db.query('INSERT into users (name, password) VALUES ($1, $2) RETURNING id', [ name, password ], (err, result) => {
          if (err) return res.status(500).end('fatal error');
    ...
    app.post('/login', (req, res) => {
      ...
      this.db.query("select id, password from users where name = $1", [name], (err, result) => {
        ...
        bcrypt.compare(password, hash, (err, res) => {
          if (err) return res.status(500).end('fatal error');
          if (!res) return res.status(401).end('unauthorized');
