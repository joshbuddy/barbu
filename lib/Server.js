  ...
  constructor() {
    ...
    app.post('/users', (req, res) => {
      ...
      bcrypt.hash(password, 8, (err, hash) => {
        ...
        this.db.query('INSERT into users (name, password) VALUES ($1, $2) RETURNING id, name', [ name, hash ], (err, result) => {
          if (err) {
            switch (err.code) {
              // unique violation
              case '23505':
    ...
    app.post('/login', (req, res) => {
      ...
      if (!name) return res.status(400).end('no name given');
      if (!password) return res.status(400).end('no password given');

      this.db.query("select id, name, password from users where name = $1", [name], (err, result) => {
        if (err) return res.status(500).end('fatal error');
        if (!result.rows[0]) return res.status(400).end('no user exists');
    ...
    app.post('/chat', isAuthenticated, (req, res) => {
      ...
      if (!message) return res.status(400).end('no name provided');

      var messageHash = {
        from: { id: req.session.userId, name: req.session.userName },
        message: String(message)
      }

      this.publisher.publish(`chat`, JSON.stringify(messageHash), (err) => {
        if (err) return res.status(500).end('internal error');
        res.status(202).end('sent');
      });
    })
    ...
    app.get('/users/:name', isAuthenticated, (req, res) => {
      ...
      this.db.query('select name from users where name = $1', [name], (err, userRows) => {
        if (err) return res.status(500).end('fatal error');
        if (userRows.rows.length === 0) return res.status(404).end('not found');

        res.send({name: userRows.rows[0].name});
      });
    });
